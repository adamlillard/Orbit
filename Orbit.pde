...
void drawOrbitersh(Orbiter n) {
    drawOrbiter(n);
    if (n.getChildren.size() >= 1) {
      for (int i = 0; i < n.getChildren().size(); i++) {
      drawOrbitersh(n.getChildren.get(i));
      }
    } else {
      return;
    }
...
void updateOrbitersh(Orbiter node, double timeDelta) {
    node.updateRotation(timeDelta);
    if(node.getChildren.size() >= 1) {
       for (int i = 0; i < node.getChildren.size(); i++) {
      updateOrbitersh(node.getChildren.get(i), timeDelta);
      }
    } else {
      return;
    }

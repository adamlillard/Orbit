 ...
 *
 * All of the methods currently just return default values. You need to make them match the Javadoc comments. 
 * 
 * @author Adam Lillard
 * @version Sept. 2017
 */
...
public class Matrix {
  ...
  public Matrix dot(Matrix that) throws UndefinedMatrixOpException {
    double[][] R = new double[this.nCols()][this.nRows()];
    for (int i = 0; i < this.nRows(); i++) {
      for (int j = 0; j < this.nCols(); j++) {
        for (int k = 0; k < this.nCols(); k++) {
          R[i][j] += this.entry(i, k) * that.entry(k, j);
        }
      }
    }
   ...
   */
   public Matrix plus(Matrix that) throws UndefinedMatrixOpException {
    ...
    for (int i = 0; i < this.nRows(); i++) {
      for (int j = 0; j < this.nCols(); j++) {
        R[i][j] = this.entry(i, j) + that.entry(i, j);
      }
    }
